...
object BracelessSyntax {
  ...
  val anIfExpression_v5 =
    if 2 > 3 then
        ...
        result
  // scala 3 one-liner
  val anIfExpressions_v6 = if 2 > 3 then "bigger" else "smaller"
  ...
  class Animal: // compiler expects body of Animal
    def eat(): Unit =
      println("I'm eating")
    end eat
  ...
  // 3 spaces + 2 tabs > 2 spaces + tabs
  // 3 spaces + tab > 3 spaces + 1 tab
  // 3 tabs + 2 spaces ??? 2 tabs + 3 spaces
  def main(args: Array[String]): Unit = {
    println(computeMeaningOfLife(40))
  }
}
